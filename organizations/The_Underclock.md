---
organization_id: AH-ORG-UNDERCLOCK
name: The Underclock
type: Decentralized tunnel network, smuggling alliance, sanctuary system, and information exchange
aliases:
  - The Underclock Network
  - Belowtime
series: The Brass Guardian / The Aetherhaven Chronicles
canon_status: Canonical working profile
canonical_scope: Aetherhaven volumes
last_updated: 2026-08-01
primary_locations:
  - Canal District service tunnels
  - Old City passages
  - The Cauldron
  - Thirteenth Canal routes
  - Forgotten maintenance corridors
leadership:
  - No confirmed central leader
key_relationships:
  - Tamsin Pike
  - Captain Mara Voss
  - The Brass Watch
  - The Cauldron
  - The Order of the Closed Eye
  - The Ninth Guild
  - The Unwound
temporal_relevance: High
---

# The Underclock

## Canonical Summary

The Underclock is a loose alliance of smugglers, canal runners, tunnel guides, couriers, sanctuary keepers, mechanics, and information brokers operating beneath and between Aetherhaven's recognized districts.

It is not primarily malicious.

Its members help people:

- escape unjust authorities,
- move forbidden books,
- bypass dangerous checkpoints,
- navigate forgotten tunnels,
- recover confiscated property,
- and disappear when official systems have left them no lawful path.

The Underclock's knowledge comes at a price.

Its protection comes with obligations.

But once someone has been granted true sanctuary, the network does not surrender them casually.

The Underclock's greatest strength is compartmentalization.

Its greatest weakness is that a person who controls the right fragment of information can manipulate one part of the network without the others understanding the larger operation.

That weakness was exploited during the disappearance of Prototype I.

## Structure

The Underclock has no confirmed central leadership.

It operates through:

- route keepers,
- tunnel guides,
- sanctuary houses,
- message relays,
- canal runners,
- and brokers who connect people without revealing full identities.

A courier may know:

- where to collect a package,
- which mark confirms the recipient,
- and which route is safe,

without knowing:

- who hired the job,
- what the package contains,
- or why the route matters.

This protects the network from [the Brass Watch](The_Brass_Watch.md).

It also makes the Underclock vulnerable to sophisticated deception.

## Sanctuary Code

The Underclock's informal law includes:

- no forced return of a protected person,
- no trafficking under the name of sanctuary,
- no violence inside a recognized safe house,
- no revelation of a route after payment and passage,
- and no sale of a client to the authority they are fleeing.

The network is morally mixed.

Some members are criminals.

Some are rescuers.

Many are both, depending on who tells the story.

## Relationship with [the Brass Watch](The_Brass_Watch.md)

[The Brass Watch](The_Brass_Watch.md) views the Underclock as:

- a smuggling network,
- obstruction of justice,
- evidence tampering,
- and a refuge for fugitives.

[Chief Inspector Thorne](../characters/Chief_Inspector_Beatrice_Thorne.md) also knows the network occasionally:

- protects witnesses,
- moves endangered people,
- preserves records,
- and reaches places the Watch cannot.

[Thorne](../characters/Chief_Inspector_Beatrice_Thorne.md) distrusts it.

She may still negotiate with it.

The Underclock sees the Watch as capable of justice in individual cases but structurally vulnerable to Council misuse and Order manipulation.

## Relationship with [Captain Mara Voss](../characters/Captain_Mara_Voss.md)

[Mara](../characters/Captain_Mara_Voss.md) maintains selective Underclock contacts.

She uses them for:

- emergency routes,
- protected movement,
- missing-person information,
- and access to people who will not speak with uniformed officers.

This reinforces [Thorne](../characters/Chief_Inspector_Beatrice_Thorne.md)'s suspicion that [Mara](../characters/Captain_Mara_Voss.md) works outside lawful channels.

[Mara](../characters/Captain_Mara_Voss.md)'s rule remains:

> She may work with criminals. She will not knowingly help anyone harm the city.

## Relationship with [Tamsin Pike](../characters/Tamsin_Pike.md)

[Tamsin Pike](../characters/Tamsin_Pike.md) may already be connected to the Underclock through canal routes, family knowledge, or inherited obligations.

Her brass key and familiarity with unmarked locks make her valuable to the network.

Whether she is:

- a full member,
- an occasional guide,
- a protected family contact,
- or someone being prepared for a larger role

remains unresolved.

## Relationship with [the Cauldron](../locations/The_Cauldron.md)

The Underclock connects [the Cauldron](../locations/The_Cauldron.md) to:

- the Canal District,
- Old City,
- [Entertainment District](../locations/The_Entertainment_District.md),
- industrial service corridors,
- and forgotten maintenance routes.

[Cauldron](../locations/The_Cauldron.md) crews rely on the network for covert transport.

The Underclock relies on [Cauldron](../locations/The_Cauldron.md) markets, safe houses, and tunnel access.

The Cinder Regent tolerates the network because it provides routes beyond official control.

## Relationship with [the Unwound](The_Unwound.md)

The broader [Unwound](The_Unwound.md) and Underclock cooperate when:

- records must be preserved,
- activists need sanctuary,
- manual equipment must be moved,
- or people face unjust detention.

The Underclock is not subordinate to [the Unwound](The_Unwound.md).

It may transport people whose goals directly conflict with [Unwound](The_Unwound.md) beliefs.

## Relationship with [the Order of the Closed Eye](The_Order_of_the_Closed_Eye.md)

The Underclock distrusts the Order's methods even before knowing its name.

Closure resembles:

- disappearance,
- confiscation,
- erased routes,
- missing witnesses,
- and altered memories.

The Order regards the Underclock as dangerous because it preserves:

- unauthorized movement,
- oral route knowledge,
- hidden identities,
- and records outside official archives.

Their most important conflict centers on Prototype I.

## The Prototype I Extraction

The Underclock was manipulated into participating in the removal of Prototype I from Order custody.

It did not know:

- the true identity of the artifact,
- the identity of the client,
- or the larger struggle between the Order and [Ninth Guild](The_Ninth_Guild.md).

### The False Rescue

A client presented evidence suggesting a conscious captive was being moved for permanent erasure.

The evidence may have included:

- forged Closure orders,
- authentic fragments stolen from the Order,
- a manufactured witness statement,
- or a message apparently sent by the captive.

The request aligned with the Underclock's sanctuary code.

### The Extraction

An Underclock crew intercepted the Quiet Transfer and moved the sealed subject through:

- canal tunnels,
- maintenance corridors,
- lock passages,
- or routes excluded from official maps.

The crew believed it was rescuing a person or conscious machine from unlawful disappearance.

### The Handoff

The intended recipient was a [Ninth Guild](The_Ninth_Guild.md) front.

The handoff failed or changed unexpectedly.

Possible causes include:

- Prototype I awakening,
- a courier recognizing [the Black Lacuna](The_Ninth_Guild.md),
- Order pursuit,
- betrayal by the client,
- or the subject refusing transfer.

### The Aftermath

The Order blamed the Underclock.

[The Ninth Guild](The_Ninth_Guild.md) recorded the accession as incomplete.

Some Underclock members later realized they had been used.

The network may retain:

- route tokens,
- payment marks,
- false client codes,
- witness memories,
- or a fragment from the transport case.

This evidence could reveal where Prototype I went next.

## Relationship with [the Ninth Guild](The_Ninth_Guild.md)

The Underclock does not trust [the Ninth Guild](The_Ninth_Guild.md).

The Guild's use of false rescue claims violated the sanctuary code and turned the network into an unwilling participant in an artifact conflict it did not understand.

Some Underclock members want revenge.

Others fear [the Ninth Guild](The_Ninth_Guild.md) still has agents inside the network.

A few may believe Prototype I truly needed rescue and that the extraction was morally right despite the deception.

## Temporal Significance

Underclock routes sometimes survive changes to official maps.

The network preserves them through:

- route songs,
- tactile markings,
- inherited keys,
- oral instructions,
- and physical habits rather than central diagrams.

Some tunnels may shift during temporal adjustments.

Experienced guides know routes that exist only:

- at certain hours,
- after Lost Seconds,
- or when approached without carrying a synchronized clock.

The Underclock does not possess a complete theory.

It knows the city has more passages than its maps admit.

## Story Opportunities

### The Job No One Remembers

An old Underclock courier remembers moving a sealed case but cannot recall the client's face.

### The False Sanctuary Mark

Someone uses Underclock protection symbols to move a [Ninth Guild](The_Ninth_Guild.md) accession.

### The Route Token

[Tamsin](../characters/Tamsin_Pike.md) discovers a token connected to the Prototype I extraction.

### The Closed Tunnel

An Order Warden seals a route the Underclock has used for generations, claiming the passage itself is dangerous knowledge.

### The Unfinished Handoff

Amelia reconstructs the last known route of Prototype I through fragments preserved by three different couriers.

## Staged Revelations

### Safe for Early Volumes

- The Underclock moves people, information, and objects through hidden routes.
- It protects some fugitives and forbidden texts.
- It is morally mixed but not inherently malicious.
- [Mara](../characters/Captain_Mara_Voss.md) has contacts within it.
- [Tamsin](../characters/Tamsin_Pike.md) may be connected.
- [The Brass Watch](The_Brass_Watch.md) treats it as an illicit network.

### Mid-Series

- The Underclock unknowingly participated in the Prototype I extraction.
- It believed it was performing a rescue.
- The client was a [Ninth Guild](The_Ninth_Guild.md) front.
- The Order blamed the network.
- Some members preserved evidence.

### Hold for Later

- Which crew performed the extraction.
- Who forged the rescue evidence.
- Whether Prototype I communicated with the crew.
- What happened at the broken handoff.
- Where the last route ended.
- Whether someone inside the Underclock knowingly aided [the Ninth Guild](The_Ninth_Guild.md).

## Continuity Constraints

- The Underclock must not be treated as secretly complicit in [the Ninth Guild](The_Ninth_Guild.md)'s plan.
- It was manipulated through its sanctuary principles.
- The network remains morally mixed but capable of genuine protection.
- Its compartmentalization must be both strength and vulnerability.
- The broader [Unwound](The_Unwound.md) is not responsible for the theft.
- The Order blames the Underclock, creating lasting hostility.
- Some Underclock evidence must survive the operation.
- [Tamsin](../characters/Tamsin_Pike.md)'s possible connection remains unresolved until explicitly developed.
- The network must remain independent of [Mara](../characters/Captain_Mara_Voss.md), [the Unwound](The_Unwound.md), and [Cauldron](../locations/The_Cauldron.md) government.

## Open Canon Questions

1. Who accepted the Prototype I extraction job?
2. What evidence convinced the crew it was a rescue?
3. Who created the false client identity?
4. Did Prototype I communicate during transport?
5. Which courier changed the route?
6. Where did the handoff fail?
7. What evidence did the network preserve?
8. Does the Order know the Underclock was manipulated?
9. Which [Ninth Guild](The_Ninth_Guild.md) faction arranged the operation?
10. Is an Order or Guild agent still embedded within the network?
