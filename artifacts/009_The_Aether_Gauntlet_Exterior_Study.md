---
artifact_id: AH-ART-009
slate_number: 9
name: 'The Aether Gauntlet: Exterior Study'
category: Amelia and the Aether Gauntlet
canon_status: canonical-working-artifact
image_status: image-linked
last_updated: '2026-08-02'
related_markdown:
- ../story_arcs/The_Black_Catalogue_Arc.md
- ../story_arcs/The_Keeper_of_Dreams.md
canonical_images:
- ../art/AH-1-004_The_Aether_Gauntlet-Exterior_Study.png
visual_transcription_status: complete
---

# The Aether Gauntlet: Exterior Study

> **Artifact Image Slate #9** · Amelia and the Aether Gauntlet · [Artifact standard](../CANON_MARKDOWN_STANDARD.md)

## Visual Reference

![The Aether Gauntlet: Exterior Study](../art/AH-1-004_The_Aether_Gauntlet-Exterior_Study.png)

[Open full image](../art/AH-1-004_The_Aether_Gauntlet-Exterior_Study.png)

## Plate Text Transcription — Visual Evidence Only

> This section records text that is physically visible in the active image. Illegible, abraded, redacted, or distorted text is labeled rather than reconstructed. Plate text is preserved even when it conflicts with newer canon.

### Archive heading and identification

- **AETHERHAVEN ARCHIVES**
- **ARTIFACT FILE**
- **CATALOG No.** `AH-1-004`
- **The Aether Gauntlet — Exterior Study**

### Printed description

> Primary interface and receptacle for the Aether Heart. Designed and constructed by Prof. Elias Hawthorne for his daughter, Amelia Hawthorne. Purpose: Channel, protect, and amplify Aetheric power through precision mechanisms.

### Quotation

> “It is not merely a machine, but a bridge between flesh and force.”  
> — E.H.

### Date tag

- **DATE:** `3rd of Gearfall, 1121 A.H.`

### Component labels

- **Aether Pressure Regulator**
- **Aether Heart (Contained)**
- **Flexion Tendon Assemblies**
- **Articulated Digits (Brass & Steel)**
- **Aether Conduit Manifold**
- **Safety Interlock Gearing**
- **Entropy Dampers (Emitted Heat)**
- **Palm Anchor Plate**

### Construction notes

> External plating composed of layered brass, tempered steel, and unknown alloy (see below). Decorative filigree removed from final construction at Amelia’s request.

### Detail captions

- **AETHER HEART HOUSING**
- **GAUNTLET — RIGHT ARM — LATERAL ELEVATION**
- **ELBOW JOINT ASSEMBLY**
- **MATERIAL SAMPLE (UNKNOWN ALLOY)**

### Unknown-alloy note

> Non-magnetic.  
> Warm to touch.  
> Resonates faintly when exposed to Aether frequencies.  
> — E.H.

### Professor Hawthorne’s additions

- **PROF. HAWTHORNE’S ADDITIONS:**
- Reinforce thumb abductor.
- Improve heat dissipation along dorsal conduits.
- Add secondary lock to wrist housing.
- Ensure no sharp edges near wearer.

Signature: `E. Hawthorne`

### Archive control fields

- **ARCHIVE ACCESSION:** `NZA-F12-77B`
- **CLASSIFICATION:** `Restricted`
- **AUTHORIZED BY:** `E. Hawthorne`
- **DATE FILED:** `1123 A.H.`

## Complete Plate Description — Visual Evidence Only

The plate combines a large sepia photograph of the completed right-arm gauntlet with a detailed lateral technical drawing and several component studies.

The gauntlet is a full articulated mechanical arm extending from an enclosed forearm cuff to individually jointed fingers. Brass plates, steel ribs, narrow pipes, small piston rods, cable-like tendons, circular bearings, and dense gearing are layered over a darker structural frame. The Aether Heart appears as a bright blue-white circular core mounted prominently near the wrist/forearm junction beneath concentric protective rings.

The central technical elevation exposes the structure more clearly. Multiple leader lines identify pressure regulation, conduit distribution, tendon assemblies, interlock gearing, heat-damping mechanisms, the palm anchor, and articulated digits. The outer cuff contains larger cylinders, manifolds, and a reinforced mounting structure.

Three lower details show:

1. a close photograph of the glowing Aether Heart housing;
2. a measured lateral line drawing of the entire right arm;
3. a close photograph of the elbow-joint assembly.

A square sample of dark unknown alloy is pinned at lower left. The surface is scratched and riveted, with a warmer metallic corner exposed. Elias’s handwritten modifications and signature occupy a separate taped note at lower right, accompanied by a circular construction diagram and a small gear emblem.

The sheet is worn and repaired with tape. No redactions or institutional seals are visible beyond the gear emblem and archive control fields.

## Non-Visual Canon References and Story Context

This plate is authoritative for the external visual construction of Amelia Hawthorne’s Aether Gauntlet and the persistent visible placement of the Aether Heart.

The image’s statement that Elias designed and constructed the arm is an archival claim. Later canon concerning Prototype I, Prototype II, unknown components, and institutional intervention may complicate the completeness of that statement. See [The Black Catalogue Arc](../story_arcs/The_Black_Catalogue_Arc.md), [Prototype II Cabinet Photograph](011_Prototype_II_Cabinet_Photograph.md), and [The Missing Prototype I Catalog Card](012_The_Missing_Prototype_I_Catalog_Card.md).

The plate should not be used to treat Amelia as equipment. It documents the prosthetic mechanism, not ownership of the person wearing it.

## Related Canon

- [The Black Catalogue Arc](../story_arcs/The_Black_Catalogue_Arc.md)
- [The Keeper of Dreams](../story_arcs/The_Keeper_of_Dreams.md)

## Continuity Notes

- This file is authoritative for the visible content and physical presentation of the active artifact image or images.
- Linked character, organization, location, and story-arc files remain authoritative for broader history and plot context.
- Visible plate claims are not silently corrected when they conflict with later canon; discrepancies are documented explicitly.
- Material from `unused/` is excluded and was not consulted.

## TODO / Production Checklist

- [x] Active canonical art linked.
- [x] All clearly readable plate text transcribed.
- [x] Dates, signatures, seals, stamps, redactions, names, places, and identifying marks documented.
- [x] Complete visual-only plate description added.
- [x] Non-visual canon and story context separated from visual evidence.
- [ ] Add backlinks from every active profile that directly references this artifact.
- [ ] Resolve documented plate/canon discrepancies only through an explicit canon decision.
- [ ] Approve a publication-safe caption.
