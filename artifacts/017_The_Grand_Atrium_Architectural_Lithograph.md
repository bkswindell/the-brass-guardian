---
schema_version: 1
id: AH-ART-017
record_type: artifact
name: The Grand Atrium Architectural Lithograph
slug: the-grand-atrium-architectural-lithograph
aliases: []
last_updated: '2026-08-02'
canon:
  status: canonical
  scope:
  - aetherhaven-volumes
development:
  status: working
disclosure:
  level: story-sensitive
assets:
- id: plate-1
  path: art/The_Grand_Atrium.png
  role: plate
  visibility: story-sensitive
  alt: Canonical archival plate for The Grand Atrium Architectural Lithograph.
artifact:
  category: The Clockwork Gardens
production:
  slate_number: 17
  image_status: image-linked
  visual_transcription_status: complete
relationships:
- target: AH-CHAR-JUNIPER-BELL
  type: related-to
  visibility: story-sensitive
- target: AH-ORG-CONSERVANCY-LIVING-MECHANISMS
  type: related-to
  visibility: story-sensitive
- target: AH-LOC-MOON-GARDEN
  type: related-to
  visibility: story-sensitive
---
# The Grand Atrium Architectural Lithograph

> **Artifact Image Slate #17** · [The Clockwork Gardens](../locations/The_Clockwork_Gardens.md) · [Artifact standard](../docs/standards/CANON_MARKDOWN_STANDARD.md)

## Visual Reference

![The Grand Atrium Architectural Lithograph](../art/The_Grand_Atrium.png)

[Open full image](../art/The_Grand_Atrium.png)

## Plate Text Transcription — Visual Evidence Only

> This section records text that is physically visible in the active image. Illegible, abraded, redacted, or distorted text is labeled rather than reconstructed. Plate text is preserved even when it conflicts with newer canon.

### Visible text

The active image contains one clearly readable inscription on an oval brass sign mounted to the open right-hand gate:

> **THE GRAND**  
> **ATRIUM**

No catalog number, archive heading, date, signature, annotation, seal, redaction, architectural label, or measurement is visible in the current image.

## Complete Plate Description — Visual Evidence Only

The active image is a wide, cinematic interior view rather than an archival architectural lithograph.

The viewer looks through an open pair of monumental black-and-brass gates into a vast glass-domed atrium. The gates are filled with gears, circular mechanisms, scrollwork, lamps, and botanical ornament. The right gate carries the oval **THE GRAND ATRIUM** sign.

The interior contains:

- a high hemispherical glass roof supported by intricate brass ribs;
- suspended rings, gears, and orrery-like mechanisms beneath the dome;
- a large central armillary or orrery mounted on a pedestal;
- a circular flower bed surrounding the central mechanism;
- multiple clocks, including a prominent clock tower at left;
- tiered galleries and balconies;
- ornate staircases rising at the rear and right;
- warm gas or aether lamps throughout;
- dense greenery, flowers, planters, and climbing vegetation;
- polished dark stone flooring inlaid with brass circles and geometric diagrams;
- visible blue sky and distant Aetherhaven architecture beyond the glass.

The visual design is symmetrical but not rigid. Mechanical structures and living plants are interwoven throughout. Warm amber light reflects from brass and wet-looking polished stone, while cool blue daylight enters through the dome.

No hidden maintenance passages or sealed omitted doorway can be verified from the active image. Those features belong to the intended artifact concept, not the visible evidence.

## Non-Visual Canon References and Story Context

The artifact slate originally calls for an antique elevation and cutaway showing the central orrery, hidden maintenance passages, and a sealed doorway omitted from public plans. The active art currently serves as a canonical location view of [the Grand Atrium](../locations/The_Grand_Atrium.md), not as that planned technical plate.

[The Grand Atrium](../locations/The_Grand_Atrium.md) is associated with [the Clockwork Gardens](../locations/The_Clockwork_Gardens.md), [Juniper Bell](../characters/Juniper_Bell.md), the [Conservancy of Living Mechanisms](../organizations/The_Conservancy_of_Living_Mechanisms.md), and [The Moon Garden](../locations/The_Moon_Garden.md).

A future true architectural plate should receive its own image or replace the current visual only after explicit approval.

## Related Canon

- [Juniper Bell](../characters/Juniper_Bell.md)
- [Conservancy of Living Mechanisms](../organizations/The_Conservancy_of_Living_Mechanisms.md)
- [The Moon Garden](../locations/The_Moon_Garden.md)

## Continuity Notes

- This file is authoritative for the visible content and physical presentation of the active artifact image or images.
- Linked character, organization, location, and story-arc files remain authoritative for broader history and plot context.
- Visible plate claims are not silently corrected when they conflict with later canon; discrepancies are documented explicitly.
- Material from `unused/` is excluded and was not consulted.

## TODO / Production Checklist

- [x] Active canonical art linked.
- [x] All clearly readable plate text transcribed.
- [x] Dates, signatures, seals, stamps, redactions, names, places, and identifying marks documented.
- [x] Complete visual-only plate description added.
- [x] Non-visual canon and story context separated from visual evidence.
- [ ] Add backlinks from every active profile that directly references this artifact.
- [ ] Resolve documented plate/canon discrepancies only through an explicit canon decision.
- [ ] Approve a publication-safe caption.
