...
while preserving YAML, existing links and images, code, URLs, and exact artifact
visual-evidence transcriptions.
"""
...
# These may appear as full explicit aliases (for example, "The Council"), but
# must never be generated as article-free or single-word aliases.
GENERIC_SINGLE_WORDS = {
    "archive", "archives", "architect", "artifact", "bell", "between",
    "commander", "conservancy", "council", "crest", "district", "eight",
    "first", "garden", "gardens", "guild", "harbormaster", "hidden", "key",
    "keeper", "mark", "order", "passenger", "prototype", "seal", "second",
    "society", "ticket", "twelve", "union", "watch",
}

# Personal surnames that are also ordinary setting words or place terms. Full
# names and explicit aliases remain linkable, but these surnames are not derived
...
def is_safe_article_free_alias(remainder: str) -> bool:
    ...
    if not words:
        return False
    if len(words) == 1 and words[0].casefold() in GENERIC_SINGLE_WORDS:
        return False
    return True


def is_epithet(title: str) -> bool:
    """Return true for distinctive character epithets, not civic job titles."""
    ...
    if not 2 <= len(words) <= 5:
        return False
    remainder = title[4:]
    return is_safe_article_free_alias(remainder)


def build_aliases(kind: str, metadata: dict[str, object], heading: str | None, path: Path) -> tuple[str, list[Alias]]:
    ...
    # The primary documented name is always linkable.
    add(display)
    if name and name.casefold() != display.casefold():
        add(name)

    # Explicit aliases are curated canon and are therefore safe as complete phrases.
    ...
    # Character titles are only aliases when they are distinctive epithets.
    if kind == "character" and title and is_epithet(title):
        add(title)

    # Add article-free versions of proper names, but never generic single words.
    ...
    # Derive commonly used personal references from the primary character name.
    # Short personal names are case-sensitive, preventing lowercase ordinary words
    # from becoming links.
...
def discover_entities() -> list[Entity]:
    ...
    # Ambiguous aliases are removed globally. Exact display names remain available.
...
def link_plain_segment(segment: str, source: Path, entities: list[Entity]) -> str:
    # Identify candidates against the original segment and render once. This avoids
    # matching text or paths inside links inserted earlier in the same pass.
